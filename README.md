...
This is done at week 8 of the cohort. It was competed in 3 days. There are some oppurtunities to refactor.
...
In the database folder, you'll find the database creation scripts and a shell script called `create.sh`. Run `winpty sh create.sh` from the database folder in Bash to create the database. IMPORTANT: You will need to provide your password `postgres1` several times as the script executes. Continue doing so until you return back to the command line. DO NOT STOP AFTER THE INITIAL TABLES.. 
